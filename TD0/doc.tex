...
\section{Pour aller plus loin: dérivation vectorielle/matricielle}

\begin{exercice}
  Soient deux vecteurs constants $\vec{a} = (a_1,\dots,a_d) \in \R^d$ et
  $\vec{b} = (b_1,\dots,b_d) \in \R^d$.
  Calculez le gradient des fonctions:
  \begin{enumerate}
  \item $f(\vec{x}) = \vec{x}\cdot\vec{a}$,  
  \item $f(\vec{x}) = (\vec{x}\cdot\vec{a})^2$,
  \item $f(\vec{x}) = (\vec{b} - \vec{x}\cdot\vec{a})^2$. 
  \end{enumerate}
\end{exercice}
